...
class CartService {
  ...
  private async getFreshToken(): Promise<string | null> {
    try {
      // Delegate token retrieval to authService which handles readiness and refresh
      const token = await authService.getToken();
      return token;
    } catch (error) {
      console.error('Error getting fresh token from authService:', error);
      return null;
    }
  }
  ...
  // Keep the commodity cart summary in sync with the primary cart storage
  private mapItemsToCommoditiesCart(items: CartItem[]): CommoditiesCartItem[] {
  ...
  private async saveCartLocally(items: CartItem[]): Promise<void> {
    await AsyncStorage.setItem(this.CART_STORAGE_KEY, JSON.stringify(items));
    await AsyncStorage.setItem(this.COMMODITIES_CART_KEY, JSON.stringify(this.mapItemsToCommoditiesCart(items)));
  }
    ...
    try {
      ...
      if (token) {
        ...
        if (response.success && response.data?.data) {
          ...
          await this.saveCartLocally(cartItems);
      ...
      if (localItems.length > 0) {
        await this.saveCartLocally(localItems);
      }
  ...
  async addToCart(item: CartItem): Promise<ApiResponse<{ message: string }>> {
    try {
      ...
      await this.saveCartLocally(items);
  ...
  async updateQuantity(itemId: string, newQuantity: number): Promise<ApiResponse<{ message: string }>> {
    try {
      ...
      await this.saveCartLocally(updatedItems);
  ...
  async removeFromCart(itemId: string): Promise<ApiResponse<{ message: string }>> {
    try {
      ...
      await this.saveCartLocally(filteredItems);
