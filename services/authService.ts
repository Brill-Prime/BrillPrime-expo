...
class AuthService {
  ...
  constructor() {
    // Prepare a ready promise that resolves after initial onAuthStateChanged fires
    this.readyPromise = new Promise<void>((resolve) => {
      this._resolveReady = resolve;
    });

    // Listen to Firebase auth state changes and resolve readiness on first event
    if (auth) {
      const unsub = onAuthStateChanged(auth, (user: FirebaseUser | null) => {
        if (user) {
          ...
          user.getIdToken()
            ...
            .catch(async error => {
              console.warn('Failed to refresh Firebase ID token on auth state change:', error);
        ...
        // Resolve readiness on first invocation
        if (this._resolveReady) {
          this._resolveReady();
          this._resolveReady = null;
        }

        // keep listening for subsequent auth state changes
      });
      // In case onAuthStateChanged doesn't fire within a short time, resolve anyway
      setTimeout(() => {
        if (this._resolveReady) {
          this._resolveReady();
          this._resolveReady = null;
      ...
      console.warn('Firebase auth is unavailable; skipping auth state listener.');
      if (this._resolveReady) {
        this._resolveReady();
        this._resolveReady = null;
      }
  ...
  // Expose a method to wait until initial auth state has been processed
  ...
  private getFirebaseAuth(): Auth {
    if (!auth) {
      throw new Error('Firebase authentication is unavailable on this platform.');
    }
    return auth;
  }
  ...
  async signUp(data: SignUpRequest): Promise<ApiResponse<AuthResponse>> {
    try {
      ...
      const firebaseUserCredential = await createUserWithEmailAndPassword(this.getFirebaseAuth(), data.email, data.password);
  ...
  async signIn(data: SignInRequest & { role?: string }): Promise<ApiResponse<AuthResponse>> {
    try {
      ...
      const firebaseUserCredential = await signInWithEmailAndPassword(this.getFirebaseAuth(), data.email, data.password);
      const firebaseUser = firebaseUserCredential.user;
  ...
  async signInWithGoogle(role?: string): Promise<ApiResponse<AuthResponse>> {
    try {
      ...
      try {
        // Try popup first
        result = await signInWithPopup(this.getFirebaseAuth(), provider);
        ...
        if (popupError.code === 'auth/popup-blocked') {
          await signInWithRedirect(this.getFirebaseAuth(), provider);
  ...
  async checkRedirectResult(): Promise<ApiResponse<AuthResponse> | null> {
    try {
      const result = await getRedirectResult(this.getFirebaseAuth());
  ...
  async requestPasswordReset(data: ResetPasswordRequest): Promise<ApiResponse<{ message: string }>> {
    try {
      ...
      await sendPasswordResetEmail(this.getFirebaseAuth(), data.email);
  ...
  async signOut(): Promise<void> {
    try {
      ...
      await firebaseSignOut(this.getFirebaseAuth());
    } catch (error: any) {
