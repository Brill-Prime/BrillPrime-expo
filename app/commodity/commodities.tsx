...
export default function CommoditiesScreen() {
  ...
  const categories: Category[] = [
    { id: 1, value: 'fuel', name: 'Fuel & Energy', icon: 'car-outline', description: 'Petrol, Diesel, Gas' },
    { id: 2, value: 'food', name: 'Food & Beverages', icon: 'restaurant-outline', description: 'Fresh food, drinks' },
    { id: 3, value: 'groceries', name: 'Supermarket/Convenience Store', icon: 'storefront-outline', description: 'Daily essentials, groceries' },
    { id: 4, value: 'health', name: 'Medical & Health', icon: 'medical-outline', description: 'Healthcare products & services' },
    { id: 5, value: 'electronics', name: 'Electronics', icon: 'phone-portrait-outline', description: 'Tech gadgets' },
    { id: 6, value: 'apparel', name: 'Apparel & Clothing', icon: 'shirt-outline', description: 'Fashion & accessories' },
    { id: 7, value: 'beauty', name: 'Beauty, Cosmetics & Personal Care', icon: 'rose-outline', description: 'Beauty & personal care' },
    { id: 8, value: 'entertainment', name: 'Art & Entertainment', icon: 'brush-outline', description: 'Creative arts & entertainment' },
    { id: 9, value: 'education', name: 'Education', icon: 'school-outline', description: 'Educational services & materials' },
    { id: 10, value: 'events', name: 'Event Planner', icon: 'calendar-outline', description: 'Event planning services' },
    { id: 11, value: 'finance', name: 'Finance', icon: 'card-outline', description: 'Financial services' },
    { id: 12, value: 'hotel', name: 'Hotel', icon: 'bed-outline', description: 'Hospitality & accommodation' },
    { id: 13, value: 'nonprofit', name: 'Non-profit Organisation', icon: 'heart-outline', description: 'Community & social services' },
    { id: 14, value: 'oil', name: 'Oil & Gas', icon: 'water-outline', description: 'Oil & gas products' },
    { id: 15, value: 'restaurant', name: 'Restaurant', icon: 'restaurant-outline', description: 'Dining & food services' },
    { id: 16, value: 'shopping', name: 'Shopping & Retail', icon: 'bag-outline', description: 'Retail shopping' },
    { id: 17, value: 'tickets', name: 'Ticket', icon: 'ticket-outline', description: 'Event & travel tickets' },
    { id: 18, value: 'toll', name: 'Toll Gate', icon: 'car-sport-outline', description: 'Toll gate services' },
    { id: 19, value: 'vehicle-service', name: 'Vehicle Service', icon: 'build-outline', description: 'Auto repair & maintenance' },
  ];
    ...
    try {
      ...
      if (response.success && response.data) {
        const mapped = response.data.map((it: any) => ({
          productId: it.commodityId || it.productId || it.id,
          quantity: it.quantity,
          price: it.price,
          productName: it.commodityName || it.productName || it.name,
          productUnit: it.unit || it.productUnit || 'unit',
  ...
  const addToCart = async (commodity: any) => {
    try {
      ...
      // Use centralized cart service which handles backend/local logic
      const res = await cartService.addToCart(cartItem as any);
      if (!res.success) throw new Error(res.error || 'Failed to add to cart');

      // Refresh local UI state
      await updateCartCount();
      await loadCartCount();
      const fresh = await cartService.getCartItems();
      if (fresh.success && fresh.data) {
        const mapped = fresh.data.map((it: any) => ({
          productId: it.commodityId || it.productId || it.id,
          quantity: it.quantity,
          price: it.price,
          productName: it.commodityName || it.productName || it.name,
  ...
  const filteredProducts = commodities.filter(product => {
    ...
    const matchesCategory = selectedCategory === 'all'
      ? true
      : categoryValue === selectedCategory || categoryValue.includes(selectedCategory);
